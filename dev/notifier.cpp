...
void Notifier::send(Message * message, NotificationType type) {
    ...
    //std::vector<class Connection *> * node_connections = this->node_instance->get_connections();
    //if (this->node_instance->get_socket_capability() == NO_ZMQ) {
    //    for (int idx = 0; idx < (int)(node_connections->size()); idx++) {
    //        node_connections->at(idx)->send(notification.c_str());
    //    }
    //}
    ...
    if (internal_conn_type == "StreamBridge" || internal_conn_type == "TCP" || internal_conn_type == "BLE" || internal_conn_type == "LoRa") {
        ...
        this->node_instance->get_connection(idx)->send(notification.c_str());
    }
    }
